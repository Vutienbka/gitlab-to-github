...
class BuyersController < UsersController
  ...
  def send_email_invite
    ...
    if @user.present?
      flash[:alert] = t('messages.email_already_exists')
    ...
    redirect_to complete_introduce_buyers_path
  end

  def complete_introduce; end
  ...
  def search_provider
    ...
    @search = Supplier.ransack({ supplier_id_cont: params[:supplier_id] })
    @search_suppliers = @search.result.where(id: params[:supplier_id])
    else
